...
void merge(sorter_records* temp_records, int sorters, Record** records, int record_count);
 ...
 */
 int j = 0;
    ...
    merge(temp_records, sorters, records, record_count);
/* 
    for(i=0;i<record_count;i++){
        printf("%ld %s %s  %s %d %s %s %-9.2f\n", \
...
void merge(sorter_records* temp_records, int sorters, Record** records, int record_count){
    ...
    if(sorters == 1){
        for(i = 0;i<record_count;i++){
            records[i] =  temp_records[0].records[i];
        }
        
    } else {
        printf("Else here temp_records[0].size = %d \n", temp_records[0].size);
        j = k =0;
        for(i=0;i<temp_records[0].size;i++){
            if(temp_records[0].records[j] < temp_records[1].records[k]){
                records[i] = temp_records[0].records[j];
                printf("%ld %s %s  %s %d %s %s %-9.2f\n", \
        records[i]->id,records[i]->name ,records[i]->surname , \
        records[i]->home_address, records[i]->home_number, records[i]->city, records[i]->mail_sector, \
        records[i]->salary);
                ...
                records[i] = temp_records[1].records[k];
                printf("%ld %s %s  %s %d %s %s %-9.2f\n", \
        records[i]->id,records[i]->name ,records[i]->surname , \
        records[i]->home_address, records[i]->home_number, records[i]->city, records[i]->mail_sector, \
        records[i]->salary);
        ...
        if(sorters > 2){
            for(s=2;s<sorters;s++){
                for(i=0;i<temp_records[s].size;i++){
                    if(records[i] < temp_records[s].records[k]){
                        j++;
                    } else {
